...
class RemoteLibrary : public QObject {
  Q_OBJECT
  Q_PROPERTY(RemoteMusicApi *server READ server CONSTANT)
...
public:
  explicit RemoteLibrary(const QString &directory, bool restore = true,
                         QObject *parent = nullptr,
                         RemoteMusicApi *api = nullptr);
  ~RemoteLibrary() override;
  RemoteMusicApi *server() { return &m_api; }
...
private:
  ...
  std::unique_ptr<RemoteMusicApi> m_apiOwner;
  RemoteMusicApi &m_api;
  Player m_player;
