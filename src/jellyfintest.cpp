...
int exerciseJellyfin(Player &local, Jellyfin &jf, QQuickWindow *window,
  ...
  if (!check(!base.isEmpty() && !password.isEmpty(),
    ...
    return 1;
  QNetworkAccessManager fixture;
  auto fixtureGet = [&](const QString &path, const QByteArray &method = "GET",
                        const QJsonObject &body = QJsonObject{}) {
    QNetworkRequest request(QUrl(base + path));
    request.setRawHeader("Authorization", "MediaBrowser Token=\"" +
                                              qgetenv("SPUN_TEST_ADMIN_TOKEN") +
                                              "\"");
    request.setTransferTimeout(3000);
    request.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    auto *reply = fixture.sendCustomRequest(
        request, method,
        method == "GET" ? QByteArray()
                        : QJsonDocument(body).toJson(QJsonDocument::Compact));
    until([&] { return reply->isFinished(); }, 4000);
    auto result = QJsonDocument::fromJson(reply->readAll());
    reply->deleteLater();
    return result;
  };
  auto session = [&]() {
    for (const auto &value : fixtureGet("/Sessions").array()) {
      const auto row = value.toObject().toVariantMap();
      if (row.value("Client") == "Spun")
        return row;
    }
    return QVariantMap{};
  };
  ...
  check(window->property("useJellyfin").toBool() && jf.enabled(),
        "Jellyfin source selected");
  capture("jellyfin-disconnected");
  click("jellyfinConnect");
  check(until([&] {
          auto *f = find(window->contentItem(), "jellyfinAddress");
          return f && f->isVisible();
        }),
        "connection dialog opens");
  check(until([&] { return find(window->contentItem(), "jellyfinAddress")->hasActiveFocus(); }),
        "connection starts with first input focused");
  QTest::keyClick(window, Qt::Key_Tab);
  check(find(window->contentItem(), "jellyfinUsername")->hasActiveFocus(),
        "Tab moves to username");
  QTest::keyClick(window, Qt::Key_Tab, Qt::ShiftModifier);
  check(until([&] { return find(window->contentItem(), "jellyfinAddress")->hasActiveFocus(); }),
        "Shift Tab returns to server field");
  for (const auto &entry :
       QList<QPair<QString, QString>>{{"jellyfinAddress", base},
  ...
  if (auto *remember = find(window->contentItem(), "jellyfinRemember"))
    remember->setProperty("checked", false);
  click("jellyfinSignIn");
  if (!check(until([&] { return api->connected(); }),
             "real server sign-in from UI"))
    return 1;
  check(until([&] { return api->folders().size() == 2; }),
        "music libraries discovered");
  check(find(window->contentItem(), "jellyfinPassword")
            ->property("text")
            .toString()
            .isEmpty(),
  ...
  capture("jellyfin-albums");
  click("jellyfinPageActions");
  click("jellyfinSettings");
  capture("jellyfin-settings");
  click("jellyfinQuality");
  click("jellyfinQuality_192");
  check(api->bitrate() == 192, "quality menu applies selected bitrate");
  api->setBitrate(0);
  QMetaObject::invokeMethod(find(window->contentItem(), "jellyfinPanel"),
                            "closeActions");
  QString otherFolder;
  for (const auto &v : api->folders())
    if (v.toMap().value("name") == "Other Music")
      otherFolder = v.toMap().value("id").toString();
  api->setFolder(otherFolder);
  jf.show("songs");
  check(!otherFolder.isEmpty() && until([&] { return !jf.busy(); }) &&
            jf.items().size() == 1 &&
            jf.items()[0].toMap().value("title") == "Other library song",
        "library selection scopes catalog");
  api->setFolder("");
  jf.search("Fixture");
  ...
  click("jellyfinPageActions");
  click("jellyfinNewPlaylist");
  check(until([&] {
          auto *f = find(window->contentItem(), "jellyfinPlaylistName");
          return f && f->hasActiveFocus();
        }),
        "playlist dialog receives keyboard focus");
  capture("jellyfin-playlist-dialog");
  bool focusTrapped = true;
  auto *dialog = find(window->contentItem(), "jellyfinNameDialog");
  for (int i = 0; i < 8; ++i) {
    QTest::keyClick(window, Qt::Key_Tab);
  ...
  if (auto *f = find(window->contentItem(), "jellyfinPlaylistName"))
    f->setProperty("text", "Spun fixture playlist");
  click("jellyfinNameDialogAccept");
  ...
  check(qAbs(session()
                 ...
                 10000 -
             5000) < 500,
        "server receives seek position");
  player->stop();
  check(
      until([&] { return session().value("NowPlayingItem").toMap().isEmpty(); },
            5000),
      "stop ends server playback session");
  jf.playItem(first);
  jf.playItem(second);
  jf.playItem(third);
  ...
  PlayerAdaptor media(&local, nullptr, nullptr, player);
  media.setSourceWindow(window);
  check(media.metadata().value("xesam:title") == player->title(),
        "MPRIS uses Jellyfin metadata");
  media.Play();
  check(until([&] { return player->playing(); }), "MPRIS plays Jellyfin");
  media.Pause();
  check(!player->playing(), "MPRIS pauses Jellyfin");
  ...
  for (const auto &medium : QStringList{"cd", "vinyl", "cassette", "tp7"}) {
    ...
    check(window->property("deckPlayer").value<QObject *>() == player,
          "2D medium uses Jellyfin transport");
    capture("jellyfin-" + medium + "-2d");
    if (qEnvironmentVariable("QT_QUICK_BACKEND") != "software") {
      ...
      check(until([&] { return window->property("threeDActive").toBool(); }),
            "3D medium activates");
      QTest::qWait(500);
      capture("jellyfin-" + medium + "-3d");
      player->seek(2000);
      click("playButton");
      check(until([&] { return player->position() > 2200; }) &&
                player->playing(),
            "3D transport starts Jellyfin audio");
      click("playButton");
      check(!player->playing(), "3D transport pauses Jellyfin audio");
    }
  }
  ...
  if (check(mixer != nullptr, "TX-6 integration is available")) {
    ...
    mixer->reset();
    check(mixer->active(), "TX-6 routes Jellyfin in TP-7 view");
    player->play();
    check(until([&] { return mixer->meters()[0].toDouble() > 0.00001; }),
          "decoded Jellyfin audio reaches TX-6 mixer");
    player->pause();
    capture("jellyfin-tx6");
    mixer->setVisible(false);
  }
  local.setMotion(false);
  QTest::qWait(100);
  check(find(window->contentItem(), "jellyfinPanel")->opacity() == 1,
        "reduced motion leaves browser fully visible");
  local.setThreeD(false);
  fixtureGet(
      "/Playlists/" + playlistId, "POST",
      {{"Users",
        QJsonArray{
            QJsonObject{{"UserId", qEnvironmentVariable("SPUN_TEST_ADMIN_ID")},
                        {"CanEdit", true}},
            QJsonObject{{"UserId", qEnvironmentVariable("SPUN_TEST_READER_ID")},
                        {"CanEdit", false}}}}});
  ...
  check(
      until(
          [&] { return !session().value("NowPlayingItem").toMap().isEmpty(); },
          5000),
      "server receives playback start");
  api->setScrobbling(false);
  check(
      until([&] { return session().value("NowPlayingItem").toMap().isEmpty(); },
            5000),
      "disabling reporting clears session");
  player->seek(3000);
  QTest::qWait(100);
  check(session().value("NowPlayingItem").toMap().isEmpty(),
        "disabled reporting stays disabled during seek");
  player->pause();
  ...
  QFile config(temp + "/jellyfin/connection.ini");
  ...
  std::cout << "JELLYFIN_RESULT " << checks << " checks, " << failures
            << " failures" << std::endl;
  return failures ? 1 : 0;
}
