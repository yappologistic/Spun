...
- **Player bodies:** **Show player body** in Preferences adds a turntable, a portable cassette player or a CD tray with a clear lid. Mini mode keeps the compact medium. Cassette finishes include Clear, Smoke and Cream. Album changes briefly show a sleeve or case and move the lid and medium; playback starts immediately. Changing tracks within an album keeps the same medium. Reduced motion skips the loading sequence. The reverse uses a matching case or booklet for the existing album details and lyrics.
- **CD speed:** enable **500 RPM** in CD Preferences for faster visual rotation. Audio speed stays unchanged; reduced motion still applies.
